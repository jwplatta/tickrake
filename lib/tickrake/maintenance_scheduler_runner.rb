  ...
  class MaintenanceSchedulerRunner
    ...
    def run
      Tickrake::Lockfile.new("tickrake-#{@scheduled_job.name}").synchronize do
        ...
        @runtime.logger.info({ msg: "Starting maintenance scheduler job #{@scheduled_job.name}.", event: "runner_start", pid: Process.pid })
        ...
        @runtime.logger.info({ msg: "Stopped maintenance scheduler job #{@scheduled_job.name}.", event: "runner_stop", reason: @shutdown_reason || "clean_exit" })
      end
    ensure
