...
module Tickrake
  class FundamentalsSchedulerRunner
    ...
    def initialize(runtime, scheduled_job:, sleeper: Kernel)
      ...
      @sleeper = sleeper
      @job = FundamentalsJob.new(runtime, scheduled_job: scheduled_job)
      @last_run_at = nil
      @last_run_on = nil
    ...
    def run
      Tickrake::Lockfile.new("tickrake-#{@scheduled_job.name}").synchronize do
        ...
        @runtime.logger.info({ msg: "Starting fundamentals scheduler job #{@scheduled_job.name}.", event: "runner_start", pid: Process.pid })
        ...
        @runtime.logger.info({ msg: "Stopped fundamentals scheduler job #{@scheduled_job.name}.", event: "runner_stop", reason: @shutdown_reason || "clean_exit" })
      end
    ensure
    ...
    def log_iteration_failure(now, error)
      ...
      @runtime.logger.error(
        "Fundamentals scheduler #{@scheduled_job.name} iteration failed at #{now.utc.iso8601}: #{error.class}: #{error.message}"
      )
