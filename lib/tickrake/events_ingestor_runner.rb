...
module Tickrake
  class EventsIngestorRunner
    ...
    def initialize(runtime, scheduled_job:, sleeper: Kernel)
      ...
      @sleeper = sleeper
      @job = EventsIngestorJob.new(runtime, scheduled_job: scheduled_job)
      @last_run_at = nil
    ...
    def run
      Tickrake::Lockfile.new("tickrake-#{@scheduled_job.name}").synchronize do
        ...
        @runtime.logger.info({ msg: "Starting events_ingest scheduler job #{@scheduled_job.name}.", event: "runner_start", pid: Process.pid })
        ...
        @runtime.logger.info({ msg: "Stopped events_ingest scheduler job #{@scheduled_job.name}.", event: "runner_stop", reason: @shutdown_reason || "clean_exit" })
      end
    ensure
    ...
    def sleep_seconds(now)
      return [@scheduled_job.interval_seconds / 2, 15].max if in_window?(now)
    ...
    def log_iteration_failure(now, error)
      ...
      @runtime.logger.error(
        "Events ingest scheduler #{@scheduled_job.name} iteration failed at #{now.utc.iso8601}: #{error.class}: #{error.message}"
      )
