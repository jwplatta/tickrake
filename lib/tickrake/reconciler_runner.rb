...
module Tickrake
  class ReconcilerRunner
    ...
    def initialize(runtime, scheduled_job:, sleeper: Kernel)
      ...
      @sleeper = sleeper
      @job = ReconcilerJob.new(runtime, scheduled_job: scheduled_job)
      @last_run_at = nil
    ...
    def log_iteration_failure(now, error)
      ...
      @runtime.logger.error(
        "Reconciler scheduler #{@scheduled_job.name} iteration failed at #{now.utc.iso8601}: #{error.class}: #{error.message}"
      )
