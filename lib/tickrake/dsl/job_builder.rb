...
module Tickrake
  module DSL
    class JobBuilder
      ...
      def reconcile(&block)
        @reconcile_builder = ReconcileBuilder.new
      ...
      def build_reconcile_job!(schedule)
        raise Tickrake::Error, "reconciler job `#{@name}` requires a reconcile block" if @reconcile_builder.nil?
